...
const NewBlogPost = (props) => {
  ...
  const checkPostTitle = async (title) => {
    try {
      const data = await fetch(
        `http://localhost:5050/blogPosts/byTitle/${title}`, {
          headers: {
            "Auth": token
          }
        });
      ...
      try {
        ...
        if (titleIsValid) {
          const uploadedFile = await fileUpload(
            file,
            `http://localhost:5050/blogPosts/${uploadLocation}/coverUpload`
          );
          const postFormData = {
            ...formData,
  ...
  const sendEmailFunction = async () => {
    ...
    try {
      const data = await fetch("http://localhost:5050/sendMail", {
        method: "POST",
      ...
      {!isLoading && (
        <Container className="new-blog-container">
          <Form className="mt-5" encType="multipart/form-data">
            ...
            <Form.Group controlId="send-email" className="mt-3">
              <Form.Check
                type="checkbox"
                label="send email"
